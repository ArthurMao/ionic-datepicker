...
.provider('ionicDatePicker', function() {
    ...
    this.$get = ['$rootScope', '$ionicPopup', '$ionicModal', 'IonicDatepickerService', function($rootScope, $ionicPopup, $ionicModal, IonicDatepickerService) {
        ...
        $scope.dateSelected = function(selectedDate) {
            ...
            if (!$scope.mainObj.dateRangeSelect) {
                $scope.selctedDateEpoch = selectedDate.epoch;
            } else {

                // reset range
                if (!$scope.selectedDates.length || $scope.selectedDates.length > 1 || $scope.selectedDates[0] == selectedDate.epoch)
                    return $scope.selectedDates.splice(0, $scope.selectedDates.length, selectedDate.epoch);

                $scope.selectedDates.push(selectedDate.epoch);

                var tempVal = Math.min.apply(null, $scope.selectedDates);
                var maxVal = Math.max.apply(null, $scope.selectedDates);

                // Start on the next day to prevent duplicating the first date
                tempVal += 1000 * 60 * 60 * 24;
                while (tempVal < maxVal) {
                    $scope.selectedDates.push(tempVal);
        ...
        $scope.setIonicDatePickerDate = function() {
            if ($scope.mainObj.dateRangeSelect) {
                $scope.mainObj.callback($scope.selectedDates);
            } else {
                $scope.mainObj.callback($scope.selctedDateEpoch);
            }
            closeModal();
        ...
        $scope.monthChanged = function(month) {
            ...
            $scope.currentDate.setMonth(1); //Fix Day31 issue, by Mao;
        ...
        function setInitialObj(ipObj) {
            ...
            $scope.selectedDates = [];
            if ($scope.mainObj.dateRangeSelect) {
                $scope.selctedDateEpoch = $scope.mainObj.title;
            }
